local opponentChatText = {'shut the up', 'skill issue', 'faggot', 'kill yourself', 'stinky', 'dumbass.', 'INCORRECT BUZZER', 'what the fuck', 'unskill issue', ':sob:'} -- you can change le text for opponent
...
local playerChatText = {'DAH BLOW ME', 'u is skill issue', 'WARIO', 'gg blud', 'fuck', 'ok i think i have no bitches', 'bitchass', 'scrollspeed is SHIT', 'average dnb fantrack', 'ok', 'shit yourself', 'o'} -- you can change le text for bf
...
function onUpdate()
 ...
 if opponentHitNoteForText == opponentNeedToHitNote then
    ...
    setTextString('opponentTextChat', opponentChatText[getRandomInt(1,6)]) 
	opponentHitNoteForText = 0
    opponentNeedToHitNote = getRandomInt(5,50)
 end
 if playerHitNoteForText == playerNeedToHitNote then
	playerChatTextIsNeedToBeShow = true 
    setTextString('playerTextChat', playerChatText[getRandomInt(1,4)]) 
	playerHitNoteForText = 0
    playerNeedToHitNote = getRandomInt(5,50)  	
 end 
 if opponentIsShocked == 5 then
	opponentChatTextIsNeedToBeShow = true 	
    setTextString('opponentTextChat', opponentChatText[getRandomInt(8,10)]) 
	opponentIsShocked = 0
 end
 if playerIsShocked == 5 then
	playerChatTextIsNeedToBeShow = true 	 
    setTextString('playerTextChat', playerChatText[getRandomInt(5,8)]) 
	playerIsShocked = 0
 end 
end
